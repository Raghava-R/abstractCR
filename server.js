...
// Define a GET route to retrieve data by abstract ID
app.get('/api/:abstractId', (req, res) => {
    let abstractId = req.params.abstractId.trim(); // Trim the abstract ID
    
    console.log("Requested abstract ID:", abstractId);
    ...
    if (dummyData.hasOwnProperty(abstractId)) {
        // If found, return the data associated with the abstract ID
        res.json(dummyData[abstractId]);
    } else {
        // If not found, return an error message
        res.status(404).json({ error: "Abstract ID not found" });
    }
});
